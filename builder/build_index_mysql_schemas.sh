...
#backend compiling
    ...
    php ${SCRIPT_FOLDER}/../backend/index_renderer.php "${SCRIPT_FOLDER}/../backend/helper/index_sql.html" "${TARGET_FOLDER}/tmp.def" "${TARGET_FOLDER}/tmp.router" > $TARGET_FOLDER/$mod_path/$mod_name.sql 
    rm $TARGET_FOLDER/tmp.def
    rm $TARGET_FOLDER/tmp.router
done
exit 0
