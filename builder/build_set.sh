...
#frontend compiling
	php $SCRIPT_FOLDER/../frontend/set_parser.php "./${mod_path}/${mod_name}.xml" "${standard_mod_path}" > $TARGET_FOLDER/tmp.def
...
#validate result
	result=$(php ${SCRIPT_FOLDER}/../tools/validator.php set ${TARGET_FOLDER}/tmp.def)
	if [ "$result" != "YES" ]; then
...
#php backend compiling
	...
	php $SCRIPT_FOLDER/../backend/set_renderer.php "${SCRIPT_FOLDER}/../backend/set/set.html" "${TARGET_FOLDER}/tmp.def" "${TARGET_FOLDER}/tmp.router.current" "${obsolete_router_object}" > ${TARGET_FOLDER}/${mod_path}/${mod_name}.set.php
	php $SCRIPT_FOLDER/../backend/set_renderer.php "${SCRIPT_FOLDER}/../backend/set/router.html" "${TARGET_FOLDER}/tmp.def" "${TARGET_FOLDER}/tmp.router.current" "${obsolete_router_object}" > ${TARGET_FOLDER}/${mod_path}/${mod_name}.router.php
	...
	do
		...
		php $SCRIPT_FOLDER/../backend/set_renderer.php "${SCRIPT_FOLDER}/../backend/set/${component}_${media}.html" "${TARGET_FOLDER}/tmp.def" "${TARGET_FOLDER}/tmp.router.current" "" > ${TARGET_FOLDER}/${mod_path}/${mod_name}.${component}.current.php
	done
	...
	if [ $has_obsolete_router -eq 1 ]; then
		...
		do
			...
			php $SCRIPT_FOLDER/../backend/set_renderer.php "${SCRIPT_FOLDER}/../backend/set/${component}_${media}.html" "${TARGET_FOLDER}/tmp.def" "" "${TARGET_FOLDER}/tmp.router.obsolete" > ${TARGET_FOLDER}/${mod_path}/${mod_name}.${component}.obsolete.php
		done
	fi
...
cp ${SCRIPT_FOLDER}/../libs/set_loader.php $TARGET_FOLDER
exit 0
