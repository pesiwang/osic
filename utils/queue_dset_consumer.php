...
class QueueDsetConsumer{
	private $_config = null;
	private $_halt = false;
	
	public function __construct(QueueDsetConsumerConfig $config, $obsolete = false){
		$this->_config = $config;
		
		$consumerClassName = 'QueueDset' . ($obsolete ? 'Obsolete' : 'Current') . '_' . ucfirst(preg_replace('/\.([a-z])/ei', "strtoupper('\\1')", $this->_config->getModule()));
		$routerClassName = 'QueueRouter' . ($obsolete ? 'Obsolete' : 'Current') . '_' . ucfirst(preg_replace('/\.([a-z])/ei', "strtoupper('\\1')", $this->_config->getModule()));
		...
		$queueFileName = $this->_config->getDirectory() . '/' . strtolower(str_replace('.', '/', $this->_config->getModule())) . '.queue.php';
		
		if(!is_readable($consumerFileName) || !is_readable($routerFileName) || !is_readable($queueFileName))
            die('file missing on module[' . $config->getModule() . "]\n");

		require_once $this->_config->getDirectory() . '/queue_loader.php';
	...
	private function _commit($routerClassName, $idStats){
		...
		foreach($idStats as $id => $stat){
			...
			$elements = $cache->load($id);
			if(isset($elements) && is_array($elements)){
				$storage->save($id, $elements);
				++$cnt;
			}
		}
		return $cnt;
...
$config = new QueueDsetConsumerConfig();
...
new QueueDsetConsumer($config, isset($options['o']));
