...
class SetDsetConsumer{
	private $_config = null;
	private $_halt = false;
	
	public function __construct(SetDsetConsumerConfig $config, $obsolete = false){
		$this->_config = $config;
		
		$consumerClassName = 'SetDset' . ($obsolete ? 'Obsolete' : 'Current') . '_' . ucfirst(preg_replace('/\.([a-z])/ei', "strtoupper('\\1')", $this->_config->getModule()));
		$routerClassName = 'SetRouter' . ($obsolete ? 'Obsolete' : 'Current') . '_' . ucfirst(preg_replace('/\.([a-z])/ei', "strtoupper('\\1')", $this->_config->getModule()));
		...
		$setFileName = $this->_config->getDirectory() . '/' . strtolower(str_replace('.', '/', $this->_config->getModule())) . '.set.php';
		
		if(!is_readable($consumerFileName) || !is_readable($routerFileName) || !is_readable($setFileName))
            die('file missing on module[' . $config->getModule() . "]\n");

		require_once $this->_config->getDirectory() . '/set_loader.php';
        require_once $setFileName;
...
$config = new SetDsetConsumerConfig();
...
new SetDsetConsumer($config, isset($options['o']));
